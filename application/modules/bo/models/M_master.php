...
class M_master extends CI_Model{
	...
	function get_stock_take($where='',$column='',$sort='',$length='',$start=''){
		...
		$this->db->select('A.*,MAX(B.date_add) as dateadd,B.id_stock_take,B.add_by,B.not_found,C.nama_class');
		$this->db->from('tb_product as A');
	...
	function export_stocktake_master($where=''){
		/*
		 * export data untuk dijadikan master asset
		 */
		//menampilkan data stock take yang paling terakhir discan
	...
	function generate_export($to,$filename,$sql,$title,$column){
		if ($to == 'csv'){
			...
			$convert = $this->dbutil->csv_from_result($sql,$delimiter,$newline,$enclosure);
	...
	function get_user_use(){
		$this->db->select('A.nama_depan,A.company_code,A.id_user');
		$this->db->from('tb_user as A');
		$this->db->join('tb_stock_take as B','A.id_user = B.id_user','inner');
		$this->db->group_by('A.id_user');
		$this->db->order_by('A.company_code','asc');
		$sql =  $this->db->get()->result();
		return $sql;
	}
	function table_move($code,$id_user){
			$res="";					
			$dt_class= $code.$id_user;
			$res .='<div class="table-responsive">
				               <table class="table table-striped table-bordered table-hover '.$dt_class.'" value="'.base_url('bo/movement/get_records/'.$code).'">
				                      <thead>
				                           <tr><th>#</th>                                        	
				                               <th>Sub Number</th>   
				                               <th>Description</th>  
			...
			$res .='<script type="text/javascript">
						$(document).ready(function(){
							primari_table(".'.$dt_class.'");
						});
					</script>';
			return $res;		
	}
	function export_pdt_outstand($where=""){
